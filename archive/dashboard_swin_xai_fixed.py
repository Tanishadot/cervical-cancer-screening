#!/usr/bin/env python3
"""
Swin Transformer XAI Dashboard - Fixed Version
Streamlit dashboard for Swin Transformer feature visualizations
"""
...
def create_feature_visualization(model, dataset, device, sample_idx):
    """Create feature visualization for a sample."""
    ...
    try:
        ...
        image_tensor = sample[0].unsqueeze(0)
        
        # Get original image
        original_sample = dataset[sample_idx]
        if len(original_sample) >= 3:
            original_image = original_sample[2]
        else:
